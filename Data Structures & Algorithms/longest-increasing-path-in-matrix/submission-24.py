class Solution:
    def longestIncreasingPath(self, matrix: List[List[int]]) -> int:
        ...
        def dfs(r, c, prev_val):
            if out_of_bounds(r, c) or prev_val >= matrix[r][c]:
                return 0
            ...
            for dx, dy in DIR:
                nr, nc = r+dx, c+dy
                path_len = dfs(nr, nc, matrix[r][c])
                if 1 + path_len > res:
                    res = max(res, 1 + path_len)
                    best = (nr, nc)
        ...
        for r in range(ROWS):
            for c in range(COLS):
                dfs(r, c, -1) # each cell is len 1

        start_cell = max(dp.keys(), key=lambda x: dp[x][0])
        # print(f"DEBUG:")
        # for r in dp:
        #     print(r)
